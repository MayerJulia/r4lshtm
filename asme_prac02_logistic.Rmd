...
table(mwanza$npa,useNA = "ifany") #useNA shows missing values. ifany option to only show NA if the dataset contains some NA, always option to show NA even if none
```
...
glm(case~npa, family = "binomial", data = mwanza) %>%
  logistic.display() #shows OR and CI + Wald's test

glm (case ~ npa, family = "binomial", data = mwanza) #shows log(OR) and no CI
```
...
glm(case ~ npa + age1, family = "binomial", data = mwanza) %>% 
  logistic.display() #gives adjusted OR
